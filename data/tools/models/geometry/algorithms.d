...
bool intersectsRayX (bool INTERSECT_ON_EDGE = false,U)(TPoint!U p, TPoint!U a, TPoint!U b) {
...
unittest {
    alias Unit = PolarDeg;
    alias Point = TPoint!Unit, Ring = TRing!Unit;
...
bool contains (bool INTERSECT_ON_EDGE = false,U)(TRing!U r, TPoint!U p) {
...
bool contains (U)(TPolygon!U r, TPoint!U p) {
...
bool contains (U)(TMultiPolygon!U r, TPoint!U p) {
    foreach (poly; r.polygons) {
        if (poly.contains(p)) return true;
    }
    return false;
}
bool contains (U)(TGeometry!U g, TPoint!U p) {
    return g.visit!(
        (TPoint!U _) { assert(false, "invalid cannot do Point/Point contains() operation"); assert(0); return false; },
        (TPolygon!U r) => r.contains(p),
        (TMultiPolygon!U r) => r.contains(p)
    );
}
bool contains (TGeometry,U)(TAABB!U bounds, TGeometry!U geometry, TPoint!U p) {
